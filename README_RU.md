...
В Beta 3 у цели есть отдельный выключенный по умолчанию **таймер сбора**: его можно включить во вкладке «Цель», поставить дату и время окончания сбора, двигать/масштабировать в предпросмотре и менять порядок в слоях.

## Самая короткая установка для новичков

1. Скачайте файл `DonConnect.Beta3.sb` из релиза.
2. В Streamer.bot нажмите **Import**.
3. Перетащите `DonConnect.Beta3.sb` в окно импорта или откройте файл как текст и вставьте его содержимое в **Import String**.
4. Нажмите **Import**.
...
Можно импортировать Beta 3 поверх прошлой версии, но перед тестами лучше удалить старые группы/actions DonConnect и импортировать заново. Так меньше шанс поймать старые хвосты от предыдущих бет.
...
### DonationAlerts и другие площадки

1. Импортируйте `DonConnect.Beta3.sb` в Streamer.bot.
2. Найдите action `DonConnect - Подключение площадок`.
3. В панели **Triggers** нажмите правой кнопкой по **Command Trigger** и выберите **Test Trigger**.
4. В браузере откроется единая страница подключения площадок.
5. Для DonationAlerts нажмите **Создать приложение**, в DonationAlerts укажите Redirect URL http://127.0.0.1:8597/donconnect/donationalerts/callback/, затем вставьте Client ID и Client Secret в DonConnect и нажмите авторизацию.
6. Для остальных площадок вставьте токен или API-ключ и нажмите **Сохранить подключения**.
7. Вернитесь в Streamer.bot и запустите `DonConnect - Widget Editor`.

Строки площадок можно сворачивать. В заголовке каждой строки показывается текущее состояние соединения, а при ошибке внутри будет указана диагностическая причина. Из редактора виджетов на эту страницу можно перейти кнопкой **Подключить площадки**.

В Beta 3 встроенных данных приложения DonationAlerts больше нет: каждый пользователь создаёт своё приложение DA. Это безопаснее для автора и тестеров, а ключи остаются только в локальных настройках DonConnect.
...
На странице подключения рядом с площадками есть прямые ссылки в личные кабинеты и настройки API.
...
Streamlabs, Donate.Stream и deStream уже имеют реализации провайдеров. StreamEngine пока отображается как **В разработке**.
...
1. Откройте **Import**.
2. Перетащите `DonConnect.Beta3.sb` в окно импорта или откройте файл как текстовый файл.
...
- Прямое редактирование алерта, цели и таймера: элементы можно двигать, растягивать восемью ручками, поворачивать и отдельно сбрасывать. Для выравнивания доступны сетка и направляющие центра без прилипания.
...
- Титры с данными Streamer.bot, паузой, пресетами и живым редактированием стиля; состав секций определяется настройками Credits самого Streamer.bot.
